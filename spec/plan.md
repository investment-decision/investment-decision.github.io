...
### Acceptance Criteria
- [ ] Live dashboard scatter shows points across more than one quadrant over a multi-month window (no more permanent Q1-only clustering) — NOT YET VERIFIED against real data; requires a live pipeline run (real FRED key) to regenerate `data/market_indices.json`. Code change is in place and unit-tested; TC-I02 remains red until that real run happens (see T2.5 note).
